...
and approximation when they face with uncertain information about the decision
they should make. Athough these techniques are subject to error, humans still
use them. Most of the times the sources of uncertainty, e,g., missing or noisy
data, incomplete knowledge of a causality, are unpredictable or even
unavoidable. Therefore, when we design intelliegent machines, they should be
able to operate in uncertain and sometimes ambiguous environments, and yet
provide the most acceptable results for us according to the conditions. The
reasoning methods under uncertainty allow AI systems to use uncertain knowledge
and make decisions while minimizing the risk \cite{tanimoto:ai-lisp}.
...
including \textit{Bayesian Belief Networks} (probabilitic reasoning),
...
systems which are emerged from early work on practical and intuitive systems for
logical inference \cite{russell:ai-modern}. However, we do not review the
description of these approaches due to their level of importance in the reviewed
literature and time. This document continues by providing the advantages and
disadvantages of the all three major theories we mentioned above (see Section
...
context-aware services required in home environment, and proposes a systematic
...
Networks and ontology together for modeling domain knowledge and reasoning
objects in probabilistic framework for a service robot. In this work, they use
objects as context information for predicting target object being present, since
it can be occluded or small in indoor environments. In
...
effects. The network is learnt by the robot through interaction with the
...
language interaction with the user. He uses Bayesian learning method to learn
robot's decision and confidence functions.
...
human-robot interaciton \cite{tahboub:hri-dbn}, mobile-robot localization
...
Bayesian network based argumentation and decision making framework that allows
agents to utilize models of the other agents. In
\cite{grootswagers:bbn-applications} authors provide a review of Bayesian
network's applications in different domains including image interpretation and
...
reasoning under ucertainty in intelligent machines. There are some theories
explaining same or different aspects of uncertainty. All these theories relate
to the same concept of uncertainty from slightly different angle. For instance,
the Bayesian Networks rely on probability theory and assign one value to each
existing variable and relate them based on their causal relationship.
Dempster-Shafer theory as an evidential reasoning theory discuss whether there
is an evidence for a partcular belief rather than probability of its truth, and
it is designed to deal with the distinction between uncertainty and ignorance.
And, finally, the Fuzzy Logic theory has the power of dealing with ambiguous
linguistic words and still provide reasonable results gaining enough credit to
be used as an underlying concept in many industrial controllers. However, all of
these theories have their own drawbacks in dealing with their major issue,
uncertainty (see Section \ref{sec:pros-cons}). As a result, it is one's
experience and the nature of the application which help to choose any of these
approaches to gain more accurate and reasonable results for the applicaiton.

In our proposed work, we believe the sources of uncertainty are everywhere in
a collaboration. The collaboration theories in general, and specifically of
ShardPlans theory concern about teamwork and involvement of others to form an
intention, to generate or evolve the shared plan, or even to establish a single
mutual belief. To do so, one should rely on perceiving others' verbal and
nonverbal behaviors which in both cases there is certain amount of uncertanity,
ambiguity and lack of evidence. Therefore, processes involved in this domian
need to be concern about the existence of uncertainty if they want to
autonomously generate and evolve underlying structure of a collaboration
procedure. Moreover, beliefs as the foundation of the underlying emotion-driven
processes are invloved in collaboration mechanisms, e.g. appraisal, motivation.
Each individual belief includes certain amount of uncertainty with respect to
what process has formed the belief, and under what situation. For instance,
...
responses based on a false belief which could be at least mitigated by having a
mechanism to deal with uncertainty in some level. As we mentioned, the existence
of uncertainty appear in different constituents of a collaboration procesure and
it is to us where to apply the appropriate mechanism to make more stable
